...
-- 测试环境信息 t_env_info
-- ----------------------------
DROP TABLE IF EXISTS `t_env_info`;
CREATE TABLE `t_env_info` (
`fid`  bigint(20) NOT NULL COMMENT '主键' ,
`fnum`  bigint(20) NOT NULL COMMENT '编号' ,
`fipaddr`  varchar(32)  NOT NULL COMMENT 'ip地址' ,
`facsversion`  varchar(45)  NOT NULL COMMENT 'acs版本' ,
`fsofaversion`  varchar(45) NOT NULL COMMENT 'sofa版本' ,
`fcodepath`  varchar(16)  NOT NULL COMMENT '代码路径' ,
`fsshaddr`  varchar(45)  NOT NULL COMMENT 'sss地址' ,
`fdatabaseaddr`  varchar(200)  NOT NULL COMMENT '数据库地址' ,
`fsetinfo`  varchar(200)  NOT NULL COMMENT '配置说明' ,
`fleader`  varchar(50)  NOT NULL COMMENT '负责人' ,
`fuse`  varchar(100)   COMMENT '用途' ,
`fconsoleaddr`  varchar(100)   COMMENT '控制台地址' ,
`fjenkinsaddr`  varchar(100)   COMMENT 'jenkins地址' ,
`fcreate_time`  datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间' ,
`flast_update_time`  datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '最后修改时间' ,
PRIMARY KEY (`fid`),
UNIQUE INDEX `unfnum_1` (`fnum`) USING BTREE COMMENT '编号唯一'
)
ENGINE=InnoDB
DEFAULT CHARACTER SET=utf8mb4 COLLATE=utf8mb4_general_ci
COMMENT='测试环境配置信息表';